...
<div class="space-y-3">
	...
	<div class="paper-card p-3 space-y-2">
		<div class="flex items-center justify-between">
			<h3 class="text-xs font-headline font-bold text-foreground uppercase tracking-wider">{m.bartering_parley_calc_title()}</h3>
			<label class="flex items-center gap-1 text-[12px] text-muted-foreground cursor-pointer" title={m.bartering_parley_value_pack_title()}>
		...
		<div class="flex gap-2 items-center">
			<span class="text-[12px] text-muted-foreground w-14 shrink-0">{m.bartering_parley_level()}</span>
			<select bind:value={barterLevel} class="paper-input text-[12.5px] px-2 py-1 flex-1">
		...
		<div class="flex gap-2 items-center">
			<span class="text-[12px] text-muted-foreground w-14 shrink-0">{m.bartering_parley_budget()}</span>
			<input
				type="number"
				bind:value={parleyBudget}
				min="0"
				step="100000"
				class="paper-input text-[12.5px] font-mono px-2 py-1 flex-1 no-spinner"
			/>
		</div>

		<!-- Reduction summary -->
		<div class="flex gap-4 text-[12px] pt-1 border-t border-outline-variant/10">
			<span class="text-muted-foreground">{m.bartering_parley_mastery()} <span class="text-foreground font-mono">{(masteryReduction * 100).toFixed(2)}%</span></span>
			{#if hasValuePack}
				<span class="text-muted-foreground">{m.bartering_parley_plus_value_pack()} <span class="text-foreground font-mono">10%</span></span>
			{/if}
			<span class="text-muted-foreground">{m.bartering_parley_total()} <span class="text-foreground font-mono font-bold">{totalReductionPct.toFixed(2)}%</span></span>
		</div>
	</div>
	...
	{#if costs}
		<div class="paper-card p-3">
			<h3 class="text-[12px] font-headline font-bold text-muted-foreground uppercase tracking-wider mb-2">{m.bartering_parley_cost_per_barter()}</h3>
			<div class="grid grid-cols-2 gap-x-4 gap-y-1">
				{#each COST_TABLE as { label, key } (key)}
					<div class="flex items-center justify-between">
						<span class="text-[12px] text-muted-foreground">{label()}</span>
						<span class="text-[12.5px] font-mono text-foreground">{formatNumber((costs as Record<string, number>)[key])}</span>
					</div>
				{/each}
			</div>
			<div class="flex gap-4 mt-2 pt-2 border-t border-outline-variant/10 text-[12px]">
				<span class="text-muted-foreground">{m.bartering_parley_regular_barters()} <span class="text-foreground font-mono font-bold">{regularBartersAvailable}</span></span>
				<span class="text-muted-foreground">{m.bartering_parley_crow_barters()} <span class="text-foreground font-mono font-bold">{crowCoinBartersAvailable}</span></span>
			</div>
		</div>
	{/if}
	...
	<!-- Route Planner -->
	<div class="paper-card p-3 space-y-2">
		<h3 class="text-[12px] font-headline font-bold text-muted-foreground uppercase tracking-wider">{m.bartering_parley_route_planner()}</h3>
		<p class="text-[12px] text-muted-foreground/60">{m.bartering_parley_route_planner_subtitle()}</p>

		<!-- Regular Routes -->
		<div>
			<span class="text-[12px] text-muted-foreground uppercase tracking-wider">{m.bartering_parley_section_regular()}</span>
			<div class="grid grid-cols-2 gap-1 mt-1">
				{#each REGULAR_ROUTES as route}
					{@const cost = costs ? (costs as Record<string, number>)[route.costKey] : 0}
					<label class="flex items-center gap-1.5 text-[12px] cursor-pointer hover:bg-white/[0.02] px-1.5 py-1 rounded">
						...
						<span class="font-mono text-[12px] text-muted-foreground/60">{formatNumber(cost)}</span>
					</label>
				{/each}
			</div>
		</div>
		...
		<!-- Margoria Routes -->
		<div>
			<span class="text-[12px] text-muted-foreground uppercase tracking-wider">{m.bartering_parley_section_margoria()}</span>
			<div class="grid grid-cols-2 gap-1 mt-1">
		...
		<!-- Crow Coin Routes -->
		<div>
			<span class="text-[12px] text-muted-foreground uppercase tracking-wider">{m.bartering_parley_section_crow()}</span>
			<div class="grid grid-cols-2 gap-1 mt-1">
		...
		<!-- Land Coin Routes -->
		<div>
			<span class="text-[12px] text-muted-foreground uppercase tracking-wider">{m.bartering_parley_section_land()}</span>
			<div class="grid grid-cols-2 gap-1 mt-1">
		...
		<div class="pt-2 border-t border-outline-variant/10 space-y-1">
			<div class="flex items-center justify-between text-[12px]">
				...
				<span class="font-mono font-bold {totalRouteCost > 0 ? 'text-foreground' : 'text-muted-foreground'}">{formatNumber(totalRouteCost)}</span>
			</div>
			<div class="flex items-center justify-between text-[12px]">
				<span class="text-muted-foreground">{m.bartering_parley_remaining()}</span>
				<span class="font-mono font-bold {parleyAfterRoutes >= 0 ? 'text-foreground' : 'text-destructive'}">{formatNumber(parleyAfterRoutes)}</span>
			</div>
			...
			{#if parleyBudget > 0}
				...
				<div class="w-full h-2 bg-surface-lowest rounded-full overflow-hidden mt-1">
					<div
						class="h-full rounded-full transition-all duration-300 {usagePct > 90 ? 'bg-destructive' : 'bg-primary'}"
						style="width: {usagePct}%"
					></div>
				</div>
				<div class="text-[12px] text-muted-foreground/60 text-right">{m.bartering_parley_budget_pct({ pct: usagePct })}</div>
			{/if}
		</div>
	</div>
</div>
